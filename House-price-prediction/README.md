...
Before running the Streamlit app, the model and scaler must be generated. If they are not present in the root directory, follow these steps:

1. **Prepare the Data:** 
   Run the training script (or notebook) to fetch the California Housing dataset from Scikit-Learn.
2. **Pre-processing:** 
   The pipeline handles missing values and applies `StandardScaler` to the features.
3. **Serialization:** 
   The trained `RandomForestRegressor` and the `scaler` are saved as `.pkl` files using `pickle`.

```bash
# Example command if you have a training script
